...
def test():
    ...
    test_file_lst = glob.glob(os.path.join(args.test_dir, "*img*.nii.gz"))  # img file list, not include labels
    print("The number of test data: ", len(test_file_lst))
    print("file list:", test_file_lst)
    ...
    for file in test_file_lst:
        name = os.path.split(file)[1]  # ('./dataset/LPBA40/test', 'S02.delineation.skullstripped.nii.gz')，区别于 file.split("/")
        print("name:", name)  # 'S02.delineation.skullstripped.nii.gz'
        ...
        # label_file = glob.glob(os.path.join(args.test_dir, name[:3] + "*label*.nii.gz"))[0]  # label file
        ...
        save_image(pred_img, f_img, f"{name[:3]}_warped.nii.gz")
        save_image(pred_flow.permute(0, 2, 3, 4, 1)[np.newaxis, ...], f_img, f"{name[:3]}_flow.nii.gz")
        save_image(pred_label, f_img, f"{name[:3]}_label.nii.gz")
        del pred_flow, pred_img, pred_label

    print("mean(DSC): ", np.mean(DSC), "   std(DSC): ", np.std(DSC))
    print("mean(time): ", np.mean(total_time), "   std(time): ", np.std(total_time))     
