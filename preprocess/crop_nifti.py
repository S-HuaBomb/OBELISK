...
for i in range(1, 43 + 1, 1):  # len(list_of_labels)):
    ...
    label_command = './c3d/bin/c3d ' + name_label + '.nii.gz -region ' + region1 + ' -int 0 -resample 144x144x144 -type short -o label_ct' + str(
        new_id) + '.nii.gz'  # -int 0 使用最邻近插值，label 的 resample 必须使用这个
    ...
    scan_command = './c3d/bin/c3d ' + name_ct + '.nii.gz -type float -region ' + region1 + ' -resample 144x144x144 -o pancreas_ct' + str(
        new_id) + '.nii.gz'  # 默认使用线性插值
